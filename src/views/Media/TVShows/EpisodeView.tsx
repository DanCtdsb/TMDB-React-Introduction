...
export const EpisodeView = () => {
  ...
  const data = useGetData<EpisodesResponse>(
    `${TELEVISION_ENDPOINT}/${id}/season/${seasonNumber}`,
    {},
    [id, seasonNumber],
  );
  ...
  const gridDataResults = data.episodes.map((episode) => ({
    ...
    primaryText: `Ep${episode.episode_number}. ${episode.name || ""}`,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="border-t border-white/10 pt-4 flex flex-col gap-3">
        <h2 className="text-[13px] text-white/40 uppercase tracking-wider">
          Episodes
        </h2>
        <ImageGrid
          results={gridDataResults}
          onClick={(season) => {
            const item = data.episodes.find((episode) => episode.id === season);
            if (!item) {
              return null;
            }
            navigate(
              `/tv/${id}/seasons/${seasonNumber}/${item.episode_number}`,
            );
          }}
        />
      </div>
